...
#define lend Temp_namedlabel(String("lend"))
    ...
    IL(OI("br label %lloop", NULL, NULL, Targets(LL(lloop, NULL))),
    IL(LI("lloop:", lloop),
    IL(OI("%`d0 = add i32 %`s0, 1", T(b), T(a), NULL),
    IL(OI("add `d0, `s0, `s1", T(c), TL(c, T(b)), NULL),
    IL(OI("%`d0 = mul `s0, 2", T(a), T(b), NULL),
    IL(OI("`d0 = cmp slt i32 %`s0, 10", T(cond), T(a), NULL),
    IL(OI("br i1 %`s0, label %lloop, label %lend", NULL, T(cond), 
					Targets(LL(lloop, L(lend)))),
    IL(LI("lend:", lend),
    IL(OI("ret %`s0", NULL, T(c), NULL),
