...
#define lend Temp_namedlabel(String("lend"))
    ...
    IL(OI("br i1 %`s0, label %lwhiletrue, label %lwhilefalse", NULL, T(cond), Targets(LL(lwhiletrue, L(lwhilefalse)))),
    IL(LI("lwhilefalse:", lwhilefalse),
    IL(OI("ret i32 %`s0", NULL, T(m), NULL),
    ...
    IL(OI("br i1 %cond, label %liftrue, label %lifend", NULL, T(cond), Targets(LL(liftrue, L(lifend)))),
    IL(LI("lifend:", lifend),
    IL(OI("%`d0 = add i32 %`s0, 1", T(i), T(i), NULL),
    IL(OI("br label %lwhiletest", NULL, NULL, Targets(L(lwhiletest))),
    IL(LI("liftrue:", liftrue),
    IL(MI("%`d0 = add i32 %`s0, 0", T(m), T(v)),
    IL(OI("br label %lifend", NULL, NULL, Targets(L(lifend))),
