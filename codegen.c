...
static void munch_binop(Temp_temp ret, T_exp e) {
    ...
    switch (e->u.BINOP.op) {
        ...
        default: fprintf(stderr, "error in BINOP!\n"); break;
    }
    if (e->u.BINOP.left->kind == T_CONST && e->u.BINOP.right->kind == T_CONST) {
        ...
        int right = e->u.BINOP.right->u.CONST;
        emit(AS_Oper(String_format("%%`d0 = %s i64 %%`s0, %d\n", op, right),
            Temp_TempList(ret, NULL), Temp_TempList(left, NULL), NULL));
    } else {
        Temp_temp left = munchExp(e->u.BINOP.left);
        Temp_temp right = munchExp(e->u.BINOP.right);
        emit(AS_Oper(String_format("%%`d0 = %s i64 %%`s0, %%`s1\n", op),
...
static void munch_extcall(Temp_temp ret, T_exp e) {
    ...
    emit(AS_Oper(String_format("%%`d0 = call ptr @%s(%s)\n", e->u.ExtCALL.extfun, String(args)), 
...
static void munch_call(Temp_temp ret, T_exp e) {
    ...
    emit(AS_Oper(String_format("%%`d0 = call i64 %%`s0(%s)\n", String(args)), 
    ...
    switch (e->kind) {
        ...
        case T_MEM: {
            ...
            if (e->u.MEM->kind == T_CONST) {
                emit(AS_Oper(String_format("%%`d0 = inttoptr i64 %d to ptr\n", e->u.MEM->u.CONST),
        ...
        case T_NAME: {
            Temp_temp ret = Temp_newtemp();
			emit(AS_Oper(String_format("%%`d0 = ptrtoint ptr @%s to i64\n", Temp_labelstring(e->u.NAME)),
                Temp_TempList(ret, NULL), NULL, NULL));
            return ret;
        }
        case T_CONST: {
            Temp_temp ret = Temp_newtemp();
            emit(AS_Oper(String_format("%%`d0 = add i64 %d, 0\n", e->u.CONST), 
                Temp_TempList(ret, NULL), NULL, NULL));
            return ret;
        }
        case T_CALL: {
        ...
        default: fprintf(stderr, "error in munchExp!\n"); break;
    }
}
    ...
    switch (s->kind) {
        ...
        case T_CJUMP: {
            ...
            switch (s->u.CJUMP.op) {
                ...
                default: fprintf(stderr, "error in CJUMP!\n"); break;
            }
            if (!s0 && !s1) {
                emit(AS_Oper(String_format("%%`d0 = icmp %s i64 %d, %d\n", op, s->u.CJUMP.left->u.CONST, s->u.CJUMP.right->u.CONST),
                    Temp_TempList(cond, NULL), NULL, NULL)); 
            } else if (!s0) {
                emit(AS_Oper(String_format("%%`d0 = icmp %s i64 %d, %%`s0\n", op, s->u.CJUMP.left->u.CONST),
                    Temp_TempList(cond, NULL), Temp_TempList(s1, NULL), NULL)); 
            } else if (!s1) {
                emit(AS_Oper(String_format("%%`d0 = icmp %s i64 %%`s0, %d\n", op, s->u.CJUMP.right->u.CONST),
                    Temp_TempList(cond, NULL), Temp_TempList(s0, NULL), NULL)); 
            } else {
                emit(AS_Oper(String_format("%%`d0 = icmp %s i64 %%`s0, %%`s1\n", op),
        ...
        case T_MOVE: {
            if (s->u.MOVE.dst->kind == T_MEM) {
                ...
                if (s->u.MOVE.src->kind == T_CONST) {
                    emit(AS_Oper(String_format("store i64 %d, ptr %%`d0\n", s->u.MOVE.src->u.CONST),
                        Temp_TempList(l1, NULL), NULL, NULL));
                } else {
