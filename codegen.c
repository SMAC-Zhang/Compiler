...
static void munchStm(T_stm s) {
    switch (s->kind) {
        ...
        case T_LABEL: {
            emit(AS_Label(String_format("l%s:\n", Temp_labelstring(s->u.LABEL)), s->u.LABEL));
			break;
        }
        case T_JUMP: {
            emit(AS_Oper(String_format("br label %%l`j0\n"),
                NULL, NULL, AS_Targets(Temp_LabelList(s->u.JUMP.jump, NULL))));
            break;
        }
        case T_CJUMP: {
            Temp_temp cond = Temp_newtemp();
            Temp_temp s0 = munchExp(s->u.CJUMP.left);
            Temp_temp s1 = munchExp(s->u.CJUMP.right);
            char *op = NULL;
            switch (s->u.CJUMP.op) {
            ...
            emit(AS_Oper(String_format("br i1 %%`s0, label %%l`j0, label %%l`j1\n"), 
        ...
        case T_MOVE: {
            if (s->u.MOVE.dst->kind == T_MEM) {
                ...
                emit(AS_Oper(String_format("store i64 %%`d0, ptr %%`s0\n"),
